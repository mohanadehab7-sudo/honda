...
async function loadDashboardData() {
    try {
        const { data: users } = await supabaseClient.from('devices').select('*');
        const now = new Date();
        ...
        const { data: codes } = await supabaseClient.from('activation_codes').select('is_used');
        const availableCodes = codes?.filter(c => !c.is_used).length || 0;

        // جلب حالة النظام (Kill Switch)
        const { data: settings } = await supabaseClient.from('app_settings').select('is_shutdown').eq('id', 1).maybeSingle();
        const statusBanner = document.getElementById('systemStatusBanner');
        if (statusBanner) {
            if (settings && settings.is_shutdown) {
                statusBanner.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>حالة النظام: متوقف حالياً (Kill Switch)</span>';
                statusBanner.className = 'status-banner offline';
            } else {
                statusBanner.innerHTML = '<i class="fas fa-check-circle"></i><span>حالة النظام: متصل ويعمل لجميع المستخدمين</span>';
...
async function loadUsersData() {
    try {
        ...
        console.log('Users fetched successfully. Count:', data?.length || 0);
        if (data && data.length > 0) {
            console.log('Sample User Data (First Row):', JSON.stringify(data[0], null, 2));
        }
        usersData = data || [];
        applyUserFilter();
    } catch (err) {
        console.error('Catch Error (Users):', err);
...
function displayUsers(users) {
    ...
    tbody.innerHTML = users.map((u, index) => {
        ...
        // حساب حالة الاتصال (نشط إذا كان آخر ظهور خلال 5 دقائق)
        ...
        return `<tr>
            <td style="text-align:center;">${onlineIndicator}</td>
            ...
            <td style="white-space:nowrap;">
                ...
                <!-- زر الشبكة (Developer Only) -->
                <button class="table-btn net ${u.net_config_enabled ? 'active' : ''}" onclick="toggleNetConfig('${u.device_id}', ${u.net_config_enabled})" title="تفعيل ضبط الشبكة">
                    <i class="fas fa-th"></i>
                </button>
    ...
    try {
        if (pendingMessageDeviceId === '__BULK__') {
            ...
            for (const deviceId of deviceIds) {
                try {
                    await supabaseClient.from('devices')
                        .update({ message: msg })
            ...
            // إرسال فردي
            const { error } = await supabaseClient.from('devices')
                .update({ message: msg })
...
async function toggleNetConfig(deviceId, currentStatus) {
    try {
        const newStatus = !currentStatus;
        const { error } = await supabaseClient.from('devices')
            .update({ net_config_enabled: newStatus })
            .eq('device_id', deviceId);
        if (error) throw error;
        showToast(newStatus ? 'تم تفعيل ضبط الشبكة للمستخدم' : 'تم إغلاق ضبط الشبكة', 'success');
        loadUsersData();
    } catch (err) { showToast('خطأ في العملية', 'error'); }
}

async function deleteUser(deviceId) {
    if (!confirm('حذف نهائي؟ لا يمكن التراجع!')) return;
    try {
        const { error } = await supabaseClient.from('devices').delete().eq('device_id', deviceId);
        if (error) throw error;
...
async function generateCodes() {
    try {
        ...
        const { error } = await supabaseClient.from('activation_codes').insert(codes);
        if (error) throw error;
...
document.getElementById('deleteAllCodesBtn').addEventListener('click', async () => {
    ...
    try {
        const { error } = await supabaseClient.from('activation_codes').delete().eq('is_used', false);
        if (error) throw error;
        showToast('تم الحذف', 'success');
        loadCodesData(); loadDashboardData();
    } catch (err) { showToast('خطأ', 'error'); }
...
document.getElementById('resetAllUsersBtn').addEventListener('click', async () => {
    ...
    try {
        const { error } = await supabaseClient.from('devices').delete().neq('device_id', 'PLACEHOLDER');
        if (error) throw error;
...
async function loadStatsData() {
    ...
    tableBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px;">جاري جلب الإحصائيات...</td></tr>';

    try {
        const { data, error } = await supabaseClient
            .from('user_daily_stats')
            .select('*, devices(user_name)')
        ...
        tableBody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:red; padding: 20px;">خطأ في جلب البيانات</td></tr>';
    }
}

function displayStats(filteredData = null) {
    const tableBody = document.getElementById('statsTableBody');
    const data = filteredData || statsData;

    if (data.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px;">لا توجد سجلات إحصائيات حتى الآن</td></tr>';
        return;
    }

    tableBody.innerHTML = data.map(row => `
        <tr>
            <td style="font-weight:bold; color:var(--primary);">${row.stats_date}</td>
            <td>
                <div style="display:flex; flex-direction:column;">
                    <span style="font-weight:600;">${row.devices?.user_name || 'مستخدم غير معروف'}</span>
                    <small style="color:#666; font-size:10px;">${row.device_id}</small>
                </div>
            </td>
            <td><span class="badge active">${row.packets_collected}</span></td>
            <td><span class="badge active">${row.cycles_completed}</span></td>
            <td><span class="badge warning">${row.active_minutes} د</span></td>
            <td><small style="color:#888;">${new Date(row.updated_at).toLocaleString('ar-EG')}</small></td>
        </tr>
    `).join('');
}
...
function applyStatsFilter() {
    ...
    const filtered = statsData.filter(row => {
        const userName = (row.devices?.user_name || '').toLowerCase();
